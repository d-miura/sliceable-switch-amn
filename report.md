...
### ①分割
```base_slice_id```に分割元のスライスID，```into_slices_id```に分割後に生成されるそれぞれのスライスID(2個)を入力することで，REST APIによるスライスの分割機能を実現する．このとき，分割後のスライスIDは ```,``` 区切りで{分割先の複数のスライスID:スライスに加えるホストのMACアドレス}の組を入力する必要がある．  
以下に分割機能の使い方の例を示す．  
```
...
### ②結合
```base_slices_id```に結合したいスライスID（複数），```into_slice_id```に結合後に新たに生成されるスライスIDを入力することで，REST APIによるスライスの結合機能を実現する．このとき，結合したいスライスIDは ```,``` 区切りで入力する必要がある．
以下に統合機能の使い方の例を示す．  
```
